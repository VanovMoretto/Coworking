        ...
        <div className="horarios-container" style={{ left: position.x, top: position.y }}>
          <div className="control-wrapper">
            <div className="label-button-wrapper">
              ...
              <button className={`${showBack ? "hidden" : "close"}`} onClick={isCloseCliked}>X</button>
            </div>
            <div className="label-button-wrapper">
              <span className={`${showBack ? "ate" : "hidden"}`}>Até:</span>
              <button className={`${showBack ? "back" : "hidden"}`} onClick={isBackClicked}>Voltar</button>
            </div>
          </div>

          {(!initialTime ? initialTimes : finalTimes)
            .filter((time) => {
              if (!initialTime) {
                return true;
              } else {
